...
|latexmath:[ O(n^n)]
|还有一些穷举类的算法，*所需时间长度成几何阶数上涨*，这就是  latexmath:[ O(n^n)] 的**"指数级"复杂度**，甚至  latexmath:[ O(n!)] 的**"阶乘级"复杂度**。
...
当面对一个计算过程时，理论工作者就寻找这样一个代数表达式（例如CN、CN^2或CN^k），它能给出这一过程对于规模已知为N的数据来说，所需要的基本步骤数目的一个上界估计。他们称这样的表达式为这个过程的"时间复杂性函数"。多项式时间过程, 是以多项式表达式为时间复杂性函数的过程。
...
对于在工业和商业中产生的几乎所有的"指数时间过程"，即使要处理规模相当适度的数据，也要让世界上最快的计算机花上比宇宙寿命还要长的时间。
...
If P = NP (i.e., our Venn diagram dissolves into a single circle) and we obtain fast algorithms for these seemingly hard problems, then the whole digital economy would become vulnerable to collapse. +
如果 P = NP（即我们的维恩图分解为一个圆圈）并且我们获得了解决这些看似困难的问题的快速算法，那么整个数字经济将变得容易崩溃。 +

This is because much of the cryptography that secures such things as your credit card number and passwords works by shrouding private information behind computationally difficult problems that can only become easy to solve if you know the secret key. +
这是因为，保护信用卡号和密码等信息的大部分加密技术都是通过将私人信息隐藏在计算困难的问题后面，而只有知道密钥，这些问题才能变得容易解决。 +

Online security as we know it rests on unproven mathematical assumptions that crumble if P = NP. +
我们知道，在线安全依赖于未经证实的数学假设，如果 P = NP，这些假设就会崩溃。 +
 +
Amazingly, we can even cast math itself as an NP problem because we can program computers to efficiently verify proofs. +
令人惊讶的是，我们甚至可以将数学本身视为 NP 问题，因为我们可以对计算机进行编程来有效地验证证明。 +

In fact, legendary mathematician Kurt Gödel first posed the P versus NP problem in a letter to his colleague John von Neumann in 1956, and he expressed (in older terminology) that P = NP “would have consequences of the greatest importance. +
事实上，传奇数学家库尔特·哥德尔 (Kurt Gödel) 在 1956 年给同事约翰·冯·诺依曼 (John von Neumann) 的一封信中首次提出了 P 与 NP 问题，他表示（用较旧的术语）P = NP“将产生最重要的后果。 +

Namely, it would obviously mean that …​ +
也就是说，这显然意味着……​ +

the mental work of a mathematician concerning yes-or-no questions could be completely replaced by a machine.” +
数学家关于是或否问题的脑力工作可以完全被机器取代。” +

If you’re a mathematician worried for your job, rest assured that most experts believe that P does not equal NP. +
如果您是一位担心自己工作的数学家，请放心，大多数专家都认为 P 不等于 NP。 +

Aside from the intuition that sometimes solutions should be harder to find than to verify, thousands of the hardest NP problems that are not known to be in P have sat unsolved across disparate fields, glowing with incentives of fame and fortune, and yet not one person has designed an efficient algorithm for a single one of them. +
除了有时找到解决方案比验证更难的直觉之外，数以千计的最难的 NP 问题在不同的领域中尚未得到解决，这些问题在名利的激励下熠熠生辉，但没有一个人知道这些问题是否存在于 P 中。为其中的一个设计了一种有效的算法。 +
 +

Of course, gut feeling and a lack of counterexamples don’t constitute a proof. +
当然，直觉和缺乏反例并不能构成证据。 +

To prove that P is different from NP, you somehow have to rule out all potential algorithms for all of the hardest NP problems, a task that appears out of reach for current mathematical techniques. +
为了证明 P 与 NP 不同，您必须以某种方式排除所有最难的 NP 问题的所有潜在算法，这是当前数学技术似乎无法完成的任务。 +

In fact, the field has coped by proving so-called barrier theorems, which say that entire categories of tempting proof strategies to resolve P versus NP cannot succeed. +
事实上，该领域已经通过证明所谓的障碍定理来应对，该定理表明解决 P 与 NP 问题的整个类别的诱人证明策略都无法成功。 +

Not only have we failed to find a proof but we also have no clue what an eventual proof might look like. +
我们不仅找不到证据，而且也不知道最终的证据会是什么样子。
